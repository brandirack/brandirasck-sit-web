...
const MOROCCO_CITY_SET = new Set(MOROCCO_CITIES.map(normalizeCity));

function validateMoroccoCity(value) {
  const city = normalizeCity(value);
  return !!city && MOROCCO_CITY_SET.has(city);
}

function validatePhone(value) {
  const v = String(value || "").trim();
  return /^(?:\+212[67]\d{8}|0[67]\d{8})$/.test(v);
}

function validateEmail(value) {
  const v = String(value || "").trim().toLowerCase();
  return /^(?!.*\s)[a-z0-9.!#$%&'*+/=?^_`{|}~-]+@(gmail|hotmail)\.com$/.test(v);
}

function validateLead(lead) {
  const x = lead || {};
  const name = String(x.name || "").trim();
  const whatsapp = String(x.whatsapp || "").trim();
  const email = String(x.email || "").trim();
  const city = String(x.city || "").trim();

  const errors = {};
  if (name.length < 2) errors.name = "invalid_name";
  if (!validatePhone(whatsapp)) errors.whatsapp = "invalid_moroccan_phone";
  if (!email || !validateEmail(email)) errors.email = "invalid_gmail_hotmail";
  if (!validateMoroccoCity(city)) errors.city = "invalid_morocco_city";
...
function publicConfig(startDate) {
  ...
  return {
    ...
    moroccoCities: MOROCCO_CITIES
  };
}


// Automatic translation fallback for any UI/Catalog text that is not present
// in the curated dictionaries. Translation is performed server-side so no
// translation service key is exposed to the browser.
const AUTO_TRANSLATION_CACHE = new Map();
const AUTO_TRANSLATION_LIMIT = 80;

function translationTargetCode(lang) {
  const map = { ar: "ar", fr: "fr", en: "en", es: "es", darija: "ary" };
...
async function autoTranslateOne(text, lang) {
  const target = translationTargetCode(lang);
  const source = String(text || "").trim();
  if (!target || !source) return source;

  const key = translationCacheKey(lang, source);
  if (AUTO_TRANSLATION_CACHE.has(key)) return AUTO_TRANSLATION_CACHE.get(key);

  // Google Translate's lightweight endpoint is used as a fallback because
  // it requires no client-side credential. The API key, if configured,
  // remains server-side and can be used by a future provider adapter.
  const url =
    "https://translate.googleapis.com/translate_a/single" +
    `?client=gtx&sl=auto&tl=${encodeURIComponent(target)}&dt=t&q=${encodeURIComponent(source)}`;
  ...
  try {
    ...
    const translated = Array.isArray(data?.[0])
      ? data[0].map(x => Array.isArray(x) ? String(x[0] || "") : "").join("")
      : "";

    const result = translated.trim() || source;
    AUTO_TRANSLATION_CACHE.set(key, result);
    return result;
  } catch (_) {
    // Never break the Catalog if an external translation service is
    // temporarily unavailable. The original text remains visible.
    return source;
  }
}

async function autoTranslateBatch(texts, lang) {
  const unique = [...new Set((Array.isArray(texts) ? texts : [])
    .map(x => String(x || "").trim())
    .filter(x => x.length >= 2 && x.length <= 500))].slice(0, AUTO_TRANSLATION_LIMIT);

  const translations = {};
  // Keep concurrency low so opening the Catalog does not create a burst.
  const concurrency = 5;
  for (let i = 0; i < unique.length; i += concurrency) {
    const batch = unique.slice(i, i + concurrency);
    const values = await Promise.all(batch.map(t => autoTranslateOne(t, lang)));
    batch.forEach((t, j) => {
      translations[translationCacheKey(lang, t)] = values[j];
    });
  }
  return translations;
}
...
module.exports = async (req, res) => {
  ...
  if (req.method === "POST") {
    ...
    if (body.action === "validateLead") {
      return res.status(200).json(validateLead(body.lead));
    }
    ...
    if (body.action === "translate") {
      ...
      if (!["ar", "darija", "fr", "en", "es"].includes(lang)) {
        return res.status(400).json({ error: "unsupported_language" });
      }
      const translations = await autoTranslateBatch(body.texts, lang);
      return res.status(200).json({ lang, translations });
    }
